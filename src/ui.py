...
class Ui:
    ...
    def menu(self):
        ...
        start_button = tk.Button(self.menu_frame, text="Start", font=font,
                                 command=self.generation_page)
        start_button.pack()
    ...
    def generation_page(self):
        ...
        degree_dropdown = ttk.Combobox(self.generation_frame, textvariable=degree_var,
                                    values=degree_values, width=10, state="readonly")
        degree_dropdown.pack(pady=10)
        degree_dropdown.set(degree_values[0])

        midi_label = tk.Label(self.generation_frame, text="Select Source MIDI File",
                            font=("Courier", 12))
        ...
        midi_dropdown = ttk.Combobox(self.generation_frame, textvariable=midi_var,
                                    values=midi_files, width=45, state="readonly")
        midi_dropdown.pack(pady=10)
        if midi_files:
            midi_dropdown.set(midi_files[0])

        duration_label = tk.Label(self.generation_frame, text="Select Note Duration Mode",
                                  font=("Courier", 12))
        duration_label.pack(pady=10)

        duration_modes = ["Same duration for every note", "Same durations as in source MIDI file",
                          "Randomized durations based on bars in source MIDI file"]
        duration_var = tk.StringVar()
        duration_dropdown = ttk.Combobox(self.generation_frame, textvariable=duration_var,
                                        values=duration_modes, width=45, state="readonly")
        duration_dropdown.pack(pady=10)
        duration_dropdown.set(duration_modes[0])

        instrument_label = tk.Label(self.generation_frame, text="Select Instrument",
                                    font=("Courier", 12))
        ...
        instrument_dropdown = ttk.Combobox(self.generation_frame, textvariable=instrument_var,
                                           values=instruments, width=20, state="readonly")
        ...
        generate_button = tk.Button(
            ...
            text="Generate Music",
            font=("Courier", 16),
            command=lambda: self.generate_music(degree_var.get(),
            midi_var.get(), duration_var.get(), instrument_var.get()))
